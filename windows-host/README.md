...
5. 以後、左の全画面が外れても約 1 秒で戻る。自分で解除するには **ESC を 1 秒長押し** か **Alt+F11**
   (コンソールの中にフォーカスがあっても効く)。自分で解除したときは自動で戻さない (再固定は Alt+F11)。
   **Ctrl+Alt+K を 2 秒長押し**で EdgeBox の画面を収納 (最小化)、もう一度の長押しで左画面の全画面に戻る
...
**EdgeBox の画面を収納する** には **Ctrl+Alt+K を 2 秒長押し**します (全画面を解除して最小化。
左画面は Windows として使える)。もう一度 **Ctrl+Alt+K を 2 秒長押し**すると最小化を解除し、
左画面の全画面に固定し直します (キーは設定ファイルの `LeftGuardStowHotkey` で変更可。長押し時間は 2 秒固定)。

> キーはコンソールの中にフォーカスがあっても効きます。コンソールの中にキー入力が入っている間、
> vmconnect は自分のフックでキーを EdgeBox へ渡してから捨てるため、以前の方式 (GetAsyncKeyState) では
> 見張り役からキーが見えず、ESC 長押しが効きませんでした。今は見張り役が低レベルキーボードフックを
> 短い間隔で入れ直して vmconnect より先にキーを受け取り (受け取るだけで EdgeBox にも届く)、
> 長押しの判定もフック側の専用スレッドで行うので、見回りが重いときでも取りこぼしません。
> フックを入れられない環境では従来どおり Windows 側を操作しているときだけ効きます
> (`display-log.txt` の「キー監視:」で確認できます)。
