...
自分で解除するには ESC を 1 秒長押しか Alt+F11 (Windows 側を操作しているときに効く。コンソール内にフォーカスがあるとキーは EdgeBox に渡る)。
自分で解除したときは自動で戻さない。再固定は Alt+F11。
左画面を一時的に Windows で使うときは Ctrl+Alt+K を 1 秒長押しでコンソールを収納 (窓を閉じる。EdgeBox は動き続ける)、もう一度長押しで全画面に戻す。
戻している間は左画面を「画面表示を戻しています」の黒い画面で覆うため、組み立て中の小さい窓は見えず、覆いが外れた時点で全画面になっている (数秒かかる。右画面はそのまま使える)。
電源 ON のときの覆いは「EdgeBox 起動中」。どちらの文字も同じ仕組み (覆い) で、EdgeBox 自体は動いたまま。
...
アイコン用のタスクは「時間制限なし・多重起動可」で登録する (表示処理が見張り役などの常駐プロセスを残すため、
タスクは「実行中」のままになる。旧設定のままだと 1 時間後に常駐プロセスごと止められ、
「実行中」の間はアイコンを押しても何も起きない)。旧版で作ったアイコンは `-Setup` を一度やり直すか、
表示処理が動いたときに設定だけ自動で直る。
...
```powershell
.\12-windows-update.ps1            # 止める (見張り役も登録)
.\12-windows-update.ps1 -Status    # 今の状態
.\12-windows-update.ps1 -UpdateNow # 更新を当てたいとき (一時解除して更新画面を開く)
.\12-windows-update.ps1 -Restore   # 既定に戻す
```

更新の途中で勝手に再起動されると収集が止まるため、5 段で止める。
Windows は設定を自分で修復するので、ポリシーだけでは足りない:

| 段 | 内容 |
|---|---|
| 1 | ポリシー: 自動更新をしない / 使用中は再起動しない / 今の版に固定 |
| 2 | サービス: wuauserv・UsoSvc・WaaSMedicSvc (修復役) を停止して無効化 |
| 3 | 予約タスク: 更新の巡回・再起動を仕掛けるタスクを無効化 |
| 4 | 見張り役: 毎日 12:00 と PC 起動時に 1〜3 を掛け直す |
| 5 | 先送り: 品質更新 30 日・大型アップデート 365 日 |

> 止めているのは**自動**更新。更新をずっと当てない運用は安全ではないため、
> 月に一度など EdgeBox を止めてよい日を決めて `-UpdateNow` で当てる。
> 更新サービスを止めるため、Microsoft Store の更新も止まる。

### EdgeBox のメモリが足りているかを測る (13-edgebox-memory.ps1)

```powershell
.\13-edgebox-memory.ps1            # 今の状態 (測定できるかどうかも表示)
.\13-edgebox-memory.ps1 -Measure   # 測定開始 (動的メモリへ。下限は今と同じ)
.\13-edgebox-memory.ps1 -Report    # 1 週間ほど使ってから結果を見る
.\13-edgebox-memory.ps1 -Fix -GB 12  # 決めた量で固定に戻す
```

メモリが「固定」の間は、外からは渡した量しか分からない (中の使用量は見えない)。
動的メモリに切り替えると Hyper-V が EdgeBox から**要求量・圧力・内部の空き**を受け取れる。
測定中の下限は今と同じなので**今より減ることはない**。5 分ごとに自動記録し、
`-Report` が最大要求量の 1.3 倍を目安に必要量を提示する。

> 測定中はメモリが動的になるため、SR-IOV のメモリ固定は外れる。
> `-Fix` で固定に戻したあと `.\11-io-passthrough.ps1 -Apply` をやり直すこと。
> EdgeBox の OS が Hyper-V への報告に対応していない場合は要求量が 0 のままになる。
> そのときは外から中を知る手段が無いため、余裕をみた割り当てで判断する。
