...
**反映のタイミング**: メモリは VM の停止中にしか変更できません。
VM が実行中のときは「停止 → 設定 → 起動」を行うか確認が出ます (収集が数分止まります)。
停止は正常シャットダウンだけを試み、**強制電源断は行いません** (シャットダウンできない場合は
何も変更せず中止します)。

```powershell
.\cpu-partition.ps1 -MemoryGB 12              # VM 停止中なら即反映。実行中なら案内だけ
