...
EdgeBox 実行中は CPU グループへの割り当てが失敗する (実機: 0x80048007) ため、割り当ては EdgeBox の停止中
(起動前) に行い、実行中の確認では固定済みのグループに触りません (ほどくと固定が外れる瞬間ができるため)。
...
各論理 CPU の負荷と、メモリの使用量を毎秒表示する別ウィンドウです。
...
```
 ...
 分離の状態: ○ 混ざっていません   EdgeBox が Windows 用コアで動いた割合 0.0% / Windows が EdgeBox 用コアで動いた割合 0.4%

 Windows 用 (CPU 0-15,22-27 / 22 論理)
  CPU 0   CPU 1   CPU 2  ...  CPU 15  CPU 22 ... CPU 27      ← 番号順。上に番号と P0/E3 の印、下に使用率
   P0      P0      P1          P7      E6         E11
   ▇       ▇       ▇           ▇       ▇          ▇
   12%     3%      8%          2%      1%         0%
 EdgeBox 用 (CPU 16-21 / 6 論理)
  CPU 16  CPU 17  CPU 18  CPU 19  CPU 20  CPU 21
   E0      E1      E2      E3      E4      E5
   ▇       ▇       ▇       ▇       ▇       ▇
   45%     38%     40%     42%     37%     41%
...
| 要素 | 意味 |
|---|---|
| **コア構成** | P コア / E コア の数と CPU 番号。CPU 自身から取得する (設定コンソールを開いていなくても出る) |
| **分離の状態** | 「EdgeBox が Windows 用コアで動いた割合」と「Windows が EdgeBox 用コアで動いた割合」。どちらかが 0.5% を超えると「△ 混ざっています」(赤)。判定は EdgeBox の仮想プロセッサ (VP) の合計実行時間と EdgeBox 用 CPU の実行時間の合計を突き合わせて求める (LP ごとの差し引きは、minroot + classic/core ではルート VP が Windows 用 CPU の中を動き回るため見かけの値が出る)。直近 3 秒の平均 |
| **区画** | 割り当ての計画に従って Windows 用 / EdgeBox 用に分けて表示。それぞれ論理 CPU の番号順 |
| **CPU の下の使用率** | その論理 CPU の合計使用率。区画に合わない側 (Windows 用で EdgeBox、EdgeBox 用で Windows) が 1% を超えていれば赤 |
...
ここでは **どの CPU を誰が使っているか** が番号順に分かり、EdgeBox が指定した独立のコアだけで
動いているか (Windows と混ざっていないか) を「分離の状態」の行と赤い数字で確かめられます。
