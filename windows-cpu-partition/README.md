...
```
 ...
 分離の状態: ○ 混ざっていません   EdgeBox が Windows 用コアで動いた割合 0.0%  /  Windows が EdgeBox 用コアで動いた割合 0.0%   (VP 合計で判定・直近 60 秒)
 完全分割 (full): 成立 — Windows は CPU 0-19 に封じ込め (minroot) / EdgeBox は CPU 20-27 に固定 (CPU グループ)
...
| **完全分割 / 締め出し** | full の成立状態 (成立 / 再起動待ち / 準分割)、runtime なら常駐の状態 |
