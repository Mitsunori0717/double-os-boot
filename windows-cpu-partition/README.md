...
```
 この PC の CPU   Intel Core i7-14700 — 20 コア / 28 スレッド (P コア 8 + E コア 12)
                 Windows が使う CPU: 20 論理 (分離が有効)
                 完全分離: ○ 成立   Windows は CPU 0-19 に固定 / EdgeBox は CPU 20-27 に固定
 対象と方式      対象の登録名: EdgeBox   方式: ( ) 簡易分離 (再起動なし)  (●) 完全分離 (再起動 1 回。推奨)   EdgeBox 側の最低コア数: 8
...
- 適用・解除・実測・監視はすべてこの画面から実行できます。完全分離は「[この内容で適用] → 再起動」だけで完了します
- 画面の方式名は「簡易分離」= runtime、「完全分離」= full です。画面には仕組みの名前 (minroot・CPU グループなど) を出しません
...
| **この画面自身の負荷** | 上部の状態行に出る、監視画面そのものが使っている CPU (Windows が使える CPU 全体に対する %)。監視は読むだけで、閉じても分離には影響しない。EdgeBox 用 CPU では一切動かない |
...
直近 60 秒分を符号付きで足してから 0 で切ります。それでも 0.1% 程度の見かけの値が残ることがあるため、
**構造上その向きの混ざりが起こり得ない状態** のときは、0.3% 未満を測定のゆらぎとみなして 0.0% と表示します:

- 「Windows が EdgeBox 用コアで動いた割合」: Windows 側の実行単位 (ルート VP) が CPU 20-27 に 1 つも無く
  (minroot)、他の VM も無いとき。Windows はその CPU を使えない構成なので、構造上 0 です
- 「EdgeBox が Windows 用コアで動いた割合」: EdgeBox が CPU 20-27 に固定済み (完全分離: ○ 成立) のとき

構造上の保証が無い状態 (再起動待ち・固定が外れた・別の VM がある) では生の値をそのまま出すので、真の漏れは隠れません。
0 でない値が続くなら真の漏れで、記録 (`cpu-partition-log.txt`) に必ず痕跡が出ます。
