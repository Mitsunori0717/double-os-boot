...
> ⚠️ 手順 6 は飛ばさないでください。**使い終わった VM を消し忘れると、後日
> ディスクの取り合い (0x80070020) の原因になります。**
> 実際にこの構成では、旧構成の VM が残っていたために起動できない事象が起きています。
...
| 症状 | 対処 |
|---|---|
| `Start-VM` が 0x80070020 で失敗 | `.\02-start-fsbp-vm.ps1 -Repair` (ディスクの二重接続・オンライン化を自動で解消) |
