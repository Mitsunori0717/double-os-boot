...
windows-host/                        構成B: Linux 側が EdgeBox (メーカー製の専用機) の場合 (Windows をホストに反転)。左画面に EdgeBox 全画面 / 右は通常のデスクトップ
windows-cpu-partition/               (別口) Windows ホスト用 CPU コア分割ツール — full モード (minroot + CPU グループ) で Windows と EdgeBox の処理を完全分離
